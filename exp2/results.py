...
def generatePlot3D(filename):
    ...
    plot("set zrange [0:7000]")
    plot("set palette defined (0.001 'green', 0.01 'yellow', 0.9 'red')")
    plot("set pm3d")
    plot.splot(Gnuplot.File('test_results_M_elapsedtime.txt', using=(1,2,3) ))#, with_="lines linecolor palette"))
    return

generatePlot3D("test_results_M_elapsedtime.png")
